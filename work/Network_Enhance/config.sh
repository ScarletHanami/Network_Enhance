#!/system/bin/sh
# config.sh — 网络增强 v1.0 用户配置中心
#
# ⚠️ 修改点 1: 命名统一为 network_enhance / v1.0
# ⚠️ 修改点 2: 检测间隔统一为 120 秒（用户要求）
# ⚠️ 修改点 3: 新增 5G 假满格判定参数（S3 算法）
# ⚠️ 修改点 4: 新增防振荡冷却参数（用户约束 1）
# ⚠️ 修改点 5: 新增无网络死锁回退参数（用户补充要求 6）
# ⚠️ 修改点 6: 新增 4G+ 跳频防护参数（用户补充要求 2）
# ⚠️ 修改点 7: 新增 Android 版本要求
#
# 来源:
#   S1 第一步: 原模块 v6.3.0 配置
#   S3 第三步: 5G 假满格判定算法
#   用户要求: 检测间隔统一 120s
#   用户约束 1: 防振荡冷却
#   用户补充要求 2: 4G+ 跳频防护
#   用户补充要求 5: Android 14+ 版本检测
#   用户补充要求 6: 无网络死锁回退
...
# ===============================
# 9. 检测间隔（修改点 2: 用户要求统一 120 秒）
# ===============================
# 所有等级 (strong/normal/weak/critical) 均使用相同间隔
# 不再按等级区分 (原 S1 中 strong=900s/normal=600s/weak=300s/critical=300s 已废弃)
