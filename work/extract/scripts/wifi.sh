#!/system/bin/sh
# wifi.sh — 卫星地球 Pro v6.3.0 WiFi 优化工具
...
apply_wifi() {
    ...
    echo "=== 应用 WiFi 优化 (v6.3.0 OEM 兼容版) ==="
...
show_wifi_status() {
    ...
    if [ -n "$rssi" ] && [ "$rssi" != "?" ]; then
        if [ "$rssi" -gt -50 ] 2>/dev/null; then echo "  信号等级: 极好"
        elif [ "$rssi" -gt -65 ] 2>/dev/null; then echo "  信号等级: 良好"
        elif [ "$rssi" -gt -75 ] 2>/dev/null; then echo "  信号等级: 一般"
        elif [ "$rssi" -gt -85 ] 2>/dev/null; then echo "  信号等级: 较弱"
        else echo "  信号等级: 极弱"; fi
    fi
    return 0
}
