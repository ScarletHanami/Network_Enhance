...
**问题反馈**：用户截图显示 WiFi 名为 `OpenWRT_5G`，但频段显示为 2.4G。

**根因**：部分 ROM 的 `cmd wifi status` / `dumpsys wifi` 输出中，频率字段可能与其他数字字段混淆，原正则 `[0-9]+` 匹配到错误数字（如 RSSI 等级），导致频段误判。

**修复方案**：
1. **重构频段解析逻辑**：提取频率数值后统一判定，不依赖输出文本中是否包含 "5G" 字样
2. **严格匹配 4 位数频率**：频率范围 2412-5825，只匹配 `[0-9]{4}` 格式
3. **新增 `_judge_freq()` 辅助函数**：
   - 频率 >4000 → 5G
   - 频率 2000-3000 → 2.4G
   - 其他值返回空（过滤异常）
4. **7 种 grep 模式 fallback**：覆盖 cmd wifi status + dumpsys wifi 各种格式

#### Bug 2：WiFi RSSI 解析为正数
**问题反馈**：用户截图显示 WiFi RSSI 为 `5 dBm`（正常应为负数如 `-65 dBm`）。

**根因**：部分国产 ROM 的 `cmd wifi status` 输出 RSSI 等级（0-100 正数）而非 dBm（负数），原正则 `[-]?[0-9]+` 会匹配到正数等级。

**修复方案**：
1. **优先匹配负数 RSSI**：所有正则改为严格匹配 `-[0-9]+` 格式
2. **正数降级处理**：若提取到 0-100 的正数，判定为等级而非 dBm，降级使用 dumpsys wifi 的 `mRssi=` 字段
3. **>100 的正数处理**：可能是绝对值 dBm（罕见），转换为负值返回
4. **日志记录**：降级时记录 `[wifi] RSSI 提取到正数 X (可能是等级), 降级处理`

### 影响
- 5G WiFi 不再被误判为 2.4G
- RSSI 不再显示为正数等级（如 5 dBm），正确显示为负数 dBm（如 -65 dBm）
- 信号等级判定（strong/normal/weak）恢复正常工作

### 版本号更新
- `module.prop`: version=v1.1.1, versionCode=111
- `common.sh`: SE_VERSION="1.1.1", SE_VERSION_CODE="111"

### 继承功能（v1.0 + v1.0.1 + v1.1）
