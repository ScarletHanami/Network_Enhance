...
#
# ⚠️ 修改点 1: 检测间隔统一为 120 秒（用户要求）
#   所有等级 (strong/normal/weak/critical) 均使用 $MONITOR_NORMAL_INTERVAL
#   移除原 S1 中按等级区分间隔 (300s/600s/900s) 的逻辑
# ⚠️ 修改点 2: 5G 假满格自动降级逻辑（S3 算法核心）
#   - 调用 carrier.sh degrade_5g_to_4g (mode=9)
#   - 连续 N 次正常才恢复 5G (默认 3 次)
# ⚠️ 修改点 3: 防振荡冷却时间（用户约束 1）
#   - 降级后强制保持 15-30 分钟 (DOWNGRADE_COOLDOWN_SEC=1800)
#   - 冷却期结束且信号稳定优秀才允许恢复 5G
# ⚠️ 修改点 4: 与 weaknet 严格隔离（用户约束 2）
#   - weaknet 激活时跳过本轮检测
#   - 绝对禁止执行任何降级/升级/无网络回退操作
# ⚠️ 修改点 5: 无网络回退策略（用户补充要求 6）
#   - 降级到 4G 后, 连续 2 次检测 Ping 完全失败, 自动恢复 5G
#   - 避免在 4G 也彻底无网的情况下死锁
# ⚠️ 修改点 6: 4 级综合判定含 SINR 维度（S3）
# ⚠️ 修改点 7: 命名统一为 network_enhance / v1.0
#
# 来源:
#   S1 第一步: 原模块 v6.3.3 调度器框架
#   S3 第三步: 5G 假满格判定算法 + 4 级判定标准
#   用户约束 1: 防振荡冷却时间
#   用户约束 2: weaknet 严格隔离
#   用户补充要求 6: 无网络回退策略
...
# ----------------------------------------------------------------------
# 防振荡参数默认值（用户约束 1, 实际值从 config.sh 读取）
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 动态参数应用（保留 S1 框架, 修改点 1: 间隔统一 120s）
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 修改点 2+3: 5G 假满格降级处理（含防振荡冷却）
# ----------------------------------------------------------------------
# 来源: S3 5G 假满格算法 + 用户约束 1（防振荡冷却）
# 全局变量:
...
handle_fake_5g() {
    ...
    # 修改点 4: weaknet 激活时绝对禁止任何操作（用户约束 2）
...
# ----------------------------------------------------------------------
# 修改点 5: 无网络回退策略（用户补充要求 6）
# ----------------------------------------------------------------------
# 来源: 用户补充要求 6
# 逻辑: 降级到 4G 后, 如果连续 2 次检测 Ping 完全失败（不是延迟高, 是彻底不通）,
#       则自动恢复 5G, 避免 4G 也无网时死锁
# 注意: 修改点 4 - weaknet 激活时绝对禁止执行（用户约束 2）
handle_no_network_rollback() {
    # weaknet 激活时绝对禁止（用户约束 2）
    if [ -f "$WEAKNET_ACTIVE_FLAG" ]; then
        return 0
    fi
...
# ----------------------------------------------------------------------
# 主循环（修改点 1+4+5: 120s 间隔 + weaknet 隔离 + 无网络回退）
# ----------------------------------------------------------------------
run_monitor_loop() {
    ...
    while true; do
        ...
        # 修改点 4: weaknet 严格隔离（用户约束 2）
        # weaknet 激活时跳过本轮, 且绝对禁止任何降级/升级/无网络回退操作
        if [ -f "$WEAKNET_ACTIVE_FLAG" ]; then
            if [ $((loop_count % 5)) -eq 0 ]; then
        ...
        # 修改点 5: 无网络回退检查（在等级判定后, PNM 操作前）
        # 必须在 weaknet 未激活时才执行（handle_no_network_rollback 内部已检查）
        handle_no_network_rollback "$ping_ms" "$net_type"

        # 修改点 2+3: 5G 假满格降级处理（含防振荡冷却）
        # 必须在 weaknet 未激活时才执行（handle_fake_5g 内部已检查）
        handle_fake_5g
...
# ----------------------------------------------------------------------
# 启动/停止/状态管理（保留 S1 框架, 修改点 7: 命名统一）
# ----------------------------------------------------------------------
start_monitor() {
    [ "$ENABLE_MONITOR" = "true" ] || {
